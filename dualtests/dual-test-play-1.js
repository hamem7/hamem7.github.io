...
// 🌟 [جديد] playRoundWinSound — نغمة فوز مختصرة لشاشة "نتيجة الجولة" الجديدة (راجع
// renderRoundSummary أدناه)، أهدأ وأقصر عمداً من playWinSound الخاصة بالنتيجة النهائية
import { playMistakeSound, playSuccessSound, playWinSound, playDrawTickSound, playDrawLandSound, playRoundWinSound } from './dual-test-sounds.js';
...
async function finishRound() {
    ...
    // 🌟 [جديد — "جولة واحدة لكل جلسة"] تثبيت مؤشر الجولة التالية فوراً قبل الحفظ، لا عند
    // الضغط على زر المتابعة كما كان سابقاً. بطلب صريح من المعلم: المواجهة لم تعد تُلعَب كاملة
    // في جلسة واحدة — كل جولة تُلعَب في يوم، ثم تُحفَظ المواجهة "معلّقة" ويُستكمَل من الجولة
    // التالية في جلسة لاحقة (راجع نافذة "المواجهات المعلقة" في dual-test-setup.js). تثبيت
    // المؤشر هنا يضمن أن الاستكمال يبدأ من الجولة الصحيحة حتى لو أُغلقت الصفحة فجأة من شاشة
    // نتيجة الجولة نفسها بلا ضغط أي زر.
    ...
    // 🌟 [مُحدَّث] بناء شاشة "نتيجة الجولة" بتصميمها الجديد بالكامل (راجع renderRoundSummary أدناه)
    await renderRoundSummary(roundResult);

    // 🌟 [مُحدَّث بطلب صريح من المعلم] زر ما بعد الجولة:
    //   • الجولة الثالثة (الأخيرة) → كما كان: عرض النتيجة النهائية للمواجهة.
    //   • غير ذلك → "إنهاء الجلسة وحفظ التقدّم" والعودة للرئيسية. الجولة التالية لا تبدأ
    //     إطلاقاً في نفس الجلسة (كان السلوك السابق يبدأها فوراً)، ويُستكمَل منها لاحقاً
    //     بنفس الطالبَين ونفس النتائج من نافذة "المواجهات المعلقة" في شاشة الاختبارات.
    const nextBtn = document.getElementById('dtp-summary-next-btn');
    const noteEl = document.getElementById('dtp-summary-note');
    const isLastRound = match.rounds.length >= 3;
    nextBtn.textContent = isLastRound ? t('dtp_view_final_btn') : t('dtp_end_session_btn');
    if (noteEl) {
        noteEl.textContent = isLastRound
            ? ''
            : t('dtp_session_saved_note').replace('{n}', match.rounds.length + 1);
        noteEl.style.display = isLastRound ? 'none' : 'block';
    }
    nextBtn.onclick = () => {
        if (isLastRound) {
            finishMatch();
        } else {
            // التقدّم محفوظ فعلاً أعلاه (currentRoundIndex + الجولة المنتهية) — لا حاجة لأي
            // حفظ إضافي هنا، فقط الخروج لشاشة البداية
            loadSplashScreen();
        }
    };
    ...
    // 🌟 [جديد] احتفال خفيف عند وجود فائز فعلي بالجولة فقط (لا عند التعادل) — كونفيتي قصير
    // + نغمة فوز مختصرة (playRoundWinSound، أقصر وأهدأ من playWinSound الخاصة بالنتيجة
    // النهائية عمداً، حتى تبقى لحظة نهاية المواجهة هي الأقوى). كلاهما best-effort: أي فشل
    // فيهما لا يؤثر إطلاقاً على عرض النتيجة نفسها.
    if (roundWinner !== 'tie') {
        try { triggerConfetti(); } catch (e) { /* best-effort */ }
        try { playRoundWinSound(); } catch (e) { /* best-effort */ }
    }
}

// 🌟 [جديد بالكامل] بناء محتوى شاشة "نتيجة الجولة" الجديدة (طبقة ملء الشاشة — راجع
// #dtp-round-summary-view و أصناف dtp-rs- في dual-test-play.html).
//
// سبب إعادة التصميم: الشكل السابق كان بطاقة بيضاء صغيرة فيها شارة نصية واحدة للفائز، ولم
// يكن واضحاً ولا متناسقاً مع باقي شاشات الاختبارات الثنائية (لوحة الأسئلة/القرعة) — بطلب
// صريح من المعلم. الشاشة الآن أربع طبقات معلومات مرتّبة بالأهمية:
//   1) منصة تتويج: تاج + صورة الفائز داخل هالة ذهبية + اسمه بخط ضخم + فارق النقاط.
//   2) بطاقتا النقاط: بطاقة الفائز بإطار ذهبي وشارة، مع شريط نسبي يوضّح الفارق بصرياً.
//   3) تقدّم المواجهة: الجولات المكسوبة حتى الآن + نقطة لكل جولة من الثلاث.
//   4) إحصائيات الجولة: الأخطاء/المساعدة/التبديل لكل طالب (كلها من roundResult مباشرة).
//
// 🌟 افتراض صريح: "فارق النقاط" يُعرض دائماً برقم مُقرَّب لخانتين عشريتين كحد أقصى، لأن خصم
// الخطأ (MISTAKE_DEDUCTION) قد ينتج كسوراً مثل 0.5 — لا تقريب لأقرب صحيح حتى لا يظهر فارق
// 0.5 كأنه "بفارق 0" أو "بفارق 1".
async function renderRoundSummary(roundResult) {
    const winner = roundResult.roundWinner; // 'A' | 'B' | 'tie'
    const scoreA = roundResult.scoreA;
    const scoreB = roundResult.scoreB;

    document.getElementById('dtp-summary-title').textContent =
        t('dtp_summary_title').replace('{n}', roundResult.roundNumber);

    // --- مؤشر الجولات (نفس أصناف نقاط لوحة الأسئلة، بلا أي منطق جديد) ---
    const stepper = document.getElementById('dtp-summary-stepper');
    stepper.innerHTML = [1, 2, 3].map(n => {
        let cls = 'dtp-round-dot';
        if (n < roundResult.roundNumber) cls += ' dtp-round-dot-done';
        else if (n === roundResult.roundNumber) cls += ' dtp-round-dot-active';
        return `<span class="${cls}"></span>`;
    }).join('<span class="dtp-round-sep"></span>');

    // --- الصور الشخصية: نفس مصدر شاشة الترحيب (إيموجي أو صورة مرفوعة أو 👤 افتراضي) ---
    // تُجلَب هنا من جديد بدل الاعتماد على ما عُرض في شاشة الترحيب، لأن الجولة قد تكون
    // استُكمِلت بعد ريفريش (راجع الافتراض رقم 3 أعلى الملف) فتكون تلك الشاشة لم تُعرض أصلاً.
    const avatarA = await getStudentAvatar(match.studentIdA);
    const avatarB = await getStudentAvatar(match.studentIdB);
    const avatarOf = (key) => renderAvatarHTML(studentName(key), key === 'A' ? avatarA : avatarB);

    // --- 1) منصة التتويج ---
    const podium = document.getElementById('dtp-summary-podium');
    if (winner === 'tie') {
        podium.className = 'dtp-rs-podium dtp-rs-tie';
        podium.innerHTML = `
            <span class="dtp-rs-caption">${t('dtp_summary_tie_caption')}</span>
            <div class="dtp-rs-tie-avatars">
                <span class="dtp-rs-avatar dtp-rs-avatar-sm">${avatarOf('A')}</span>
                <span class="dtp-rs-tie-vs">🤝</span>
                <span class="dtp-rs-avatar dtp-rs-avatar-sm">${avatarOf('B')}</span>
            </div>
            <span class="dtp-rs-winner-name">${t('dtp_summary_tie_big')}</span>`;
    } else {
        const diff = Math.round(Math.abs(scoreA - scoreB) * 100) / 100;
        podium.className = 'dtp-rs-podium';
        podium.innerHTML = `
            <span class="dtp-rs-caption">${t('dtp_summary_winner_caption')}</span>
            <div class="dtp-rs-avatar-wrap">
    ...
    // --- 2) بطاقتا النقاط + شريط نسبي (طول الشريط = نسبة نقاط الطالب لأعلى نتيجة في الجولة) ---
    const maxScore = Math.max(scoreA, scoreB, 1); // 1 كحد أدنى لتفادي القسمة على صفر لو الجولة بلا نقاط
    const scoreCardHTML = (key) => {
        const score = key === 'A' ? scoreA : scoreB;
        const isWinner = winner === key;
        const pct = Math.max(4, Math.round((score / maxScore) * 100)); // 4% كحد أدنى ليبقى الشريط مرئياً
        return `
    ...
    const scoresEl = document.getElementById('dtp-summary-scores');
    scoresEl.innerHTML = scoreCardHTML('A') + scoreCardHTML('B');
    // عدّ تصاعدي من صفر لرقم كل طالب — إعادة استخدام animateScoreCountUp نفسها المستخدَمة
    // في شريط النقاط أثناء اللعب، بلا أي كود حركة جديد
    animateScoreCountUp(scoresEl.querySelector('[data-rs-score="A"]'), 0, scoreA);
    animateScoreCountUp(scoresEl.querySelector('[data-rs-score="B"]'), 0, scoreB);

    // --- 3) تقدّم المواجهة: الجولات المكسوبة حتى الآن (من الجولات المكتملة فعلاً فقط) ---
    // 🌟 ملاحظة: نحسبها هنا بـ computeSeriesResult على match.rounds (الجولات المنتهية)، لا من
    // match.roundsWonA/B لأن هذين الحقلين لا يُكتبان إلا في نهاية المواجهة كلها (finishMatch).
    const series = computeSeriesResult(match.rounds);
    const dotsHTML = [0, 1, 2].map(i => {
        const r = match.rounds[i];
        if (!r) {
            return `<span class="dtp-rs-series-dot" title="${t('dtp_summary_round_pending').replace('{n}', i + 1)}">${i + 1}</span>`;
        }
        if (r.roundWinner === 'tie') {
            return `<span class="dtp-rs-series-dot dtp-rs-dot-tie" title="${t('dtp_summary_round_tied').replace('{n}', i + 1)}">=</span>`;
        }
        const nm = r.roundWinner === 'A' ? match.studentNameA : match.studentNameB;
        const cls = r.roundWinner === 'A' ? 'dtp-rs-dot-a' : 'dtp-rs-dot-b';
        const title = t('dtp_summary_round_won_by').replace('{n}', i + 1).replace('{name}', nm);
        return `<span class="dtp-rs-series-dot ${cls}" title="${title}">${String(nm).trim().charAt(0)}</span>`;
    }).join('');
    ...
    // --- 4) إحصائيات الجولة (ثلاثة صفوف: أخطاء / مساعدة / تبديل) ---
    const statRow = (label, a, b) => `
        <div class="dtp-rs-stat-row">
            <span class="dtp-rs-stat-val dtp-rs-stat-a">${a}</span>
            <span class="dtp-rs-stat-label">${label}</span>
            <span class="dtp-rs-stat-val dtp-rs-stat-b">${b}</span>
        </div>`;
    const usedMark = (v) => (v ? t('dtp_summary_used_yes') : t('dtp_summary_used_no'));
    document.getElementById('dtp-summary-stats').innerHTML = `
        ...
        ${statRow(`🔄 ${t('dtp_summary_stat_swap')}`, usedMark(roundResult.swapUsedA), usedMark(roundResult.swapUsedB))}`;
}
